...
/**
 * The `layoutId` the Course detail heading should carry, or `undefined` for a plain
 * heading. Arms the morph only for a mount that painted the heading immediately —
 * `isPending` already `false` on the first render, meaning React Query served every
 * gating query from cache (ADR-0030) and the heading commits in the same pass the
 * list unmounts in, which is the only pass where framer still holds the source's
 * measured box. A mount that opened on the loading gate stays disarmed for good:
 * arming it once its queries land would pair the heading with a source that left
 * the tree several commits ago. See {@link courseMorphLayoutId} for the full rule.
 *
