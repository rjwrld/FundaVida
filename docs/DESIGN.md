...
| ------------- | --------- | ----------------------- | ----------------------------------------------------------------------------------------------------------------------- |
| Default Green | `#7CFC00` | `--color-default-green` | Green action color for filled buttons, selected navigation states, and focused conversion moments                       |
...
| Paper White   | `#ffffff` | `--color-paper-white`   | Card surfaces, elevated panels, input fields — the bright surface layer above Frost                                     |
...
Filled green (#7CFC00) background, #202020 text, 8px radius, 10–20px horizontal padding, 5px vertical padding. Saans Trial weight 500, 14–16px. Arrow icon (→) appended in #202020. 1425.6px (pill) variant for inline hero CTAs.
...
White background, #f5f5f5 or #202020 hairline border, 8px radius, 16–20px padding. Placeholder text in Carbon (#333333). Green CTA button attached to the right inside a shared container, 8px outer radius.
...
White or transparent background, sticky. Logo left, nav links center in Saans 500, Login (text link) and green CTA right. No border-bottom — separation comes from the canvas color shift.
...
- Use #7CFC00 only for primary actions, active states, and link accents — its scarcity is its power
...
- Do not introduce additional chromatic colors — the system is monochrome + one green accent, and adding more breaks the restraint
...
| 2     | Accent Fill | `#7CFC00` | Primary action buttons, active states               |
...
- Accent/primary action: #7CFC00 (filled action)
- Accent on white: #7CFC00 for marks and underlines only; use #202020 for readable text

## Example Component Prompts

1. **Hero Section**: Center on #f5f5f5 canvas. Pill announcement tag above headline (white bg, #202020 1px border, 9999px radius, 8px/16px padding, Saans 500 14px in #202020). Headline at 72px NB International Pro weight 500, #202020, line-height 0.97, letter-spacing -1.44px. Subtext at 18px Saans 500, #202020, max-width 640px centered. Email input group: white bg, #202020 1px border, 8px radius, 16px padding, with inline green (#7CFC00) button "Book a demo →" at 14px Saans 500 #202020 text, right-attached.
...
5. **Navigation Bar**: White or transparent, max-width 1200px centered. Logo left, nav links (Platform, Use cases, Pricing, Resources) in Saans 500 14px #202020 center. Right side: "Login" text link in #202020, then green (#7CFC00) pill button "Book a demo →" at 14px Saans 500 #202020 text, 9999px radius, 20px/10px padding.
...
- **Linear** — Same near-monochrome interface with a single vivid accent color (Linear's purple vs. Default's green), tight geometric display type, and bordered flat cards with no shadows
...
:root {
  ...
  --font-nb-international-pro:
    ...
    Roboto, sans-serif;
  --font-saans-trial:
    'Saans Trial', ui-sans-serif, system-ui, -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto,
    sans-serif;
